...
export const CONFIG = {
  ...
  ADS: {
    ...
    // ГЛАВНЫЙ ВЫКЛЮЧАТЕЛЬ интерстишиалов. false → межстраничную рекламу не
    // показываем нигде и никогда: ни между уровнями, ни по resume-триггеру,
    // ни preload'им её (экономим трафик игрока). Rewarded это НЕ трогает —
    // реклама за подсказки продолжает работать как раньше.
    //
    // Выключено с 2026-05-27: в отзывах игроки жалуются на рекламу между
    // уровнями, а по деньгам interstitial даёт крохи на фоне rewarded.
    // Чтобы вернуть — поставить true (расписание ниже уже настроено).
    interstitialEnabled:        false,
    // Интерстишиал-расписание (действует только при interstitialEnabled: true):
