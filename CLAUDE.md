...
Что умеет именно эта игра — [`js/testActions.js`](js/testActions.js): `state`, `level.set`, `grant.hints`, `progress.reset`. Плюс общие команды моста: параметры,
...
- Расхождение объявления с константами сборки видно в консоли:
  `warnDeclarationDrift()` в дев-сборке пишет о нём при запуске. Дефолт — это
  то, что применится при недоступном бакете, и расхождение означает «без сети
  игра ведёт себя иначе, чем с ней».
- В `cloud/config/*.json` лежат **только сознательные отклонения**; пусто —
  значит «как в сборке».
- НЕ вынесены параметры генератора уровней: уровни приезжают в сборке уже
  сгенерированными (`js/levels.generated.js`), и правка генератора из облака не
  изменила бы у игрока ничего. Ключ, который выглядит рабочим и ни на что не
  влияет, хуже отсутствующего.
