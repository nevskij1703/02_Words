...
- **`hints_start` — исключение из предыдущего пункта.** Он правит поле сейва,
  поэтому читается РОВНО ОДИН РАЗ: `storage.applyStartGrant()` зовётся из
  `bootstrap()` после ответа сети и прибавляет **разницу** к значению сборки
  (флаг `startAdjusted`, `migration[4]`). Присвоение вернуло бы подсказку,
  потраченную за те секунды, что отвечала сеть; а читать ключ в `DEFAULT_STATE`
  нельзя вовсе — `storage.load()` стоит первой строкой `bootstrap()`, раньше
  всякой сети, и вернул бы значение сборки каждому новому игроку.
